...
stat.test <- vocal_act %>% 
  ...
  wilcox_test(percent_detections ~ territory_cat) 
```

Marginal significant differences were observed between non-territorial and weakly territorial species at dawn (p = 0.039)
...
fig_terr_vocAct <- vocal_act %>% 
  ...
  mutate(territory_cat = fct_relevel(territory_cat, 
            "Highly territorial", "Weakly territorial", "Non-territorial")) %>%
  ggbetweenstats(x = territory_cat,
...
ggsave(fig_terr_vocAct, filename = "figs/fig_percentDetections_territoriality.png", width = 16, height = 14, device = png(), units = "in", dpi = 300)
dev.off() 
```
# Vocal activity is significantly higher in dawn than dusk in all territorial categories. 

![% acoustic detections at dawn across categories of territoriality](figs/fig_percentDetections_territoriality.png) 
