...
# PGLS regression (Phylogenetic Generalized Least Squares) analysis
  
In this script, we understand which driver/hypothesis (light availability, peak frequency, trophic niche, territoriality, and sociality) best explains the variation in vocal activity. We use linear regresssion to understand which of these variables best predicts the vocal activity (response variable), while controlling for potential phylogenetic signals/relationships.
...
## Vocal Activity/Percent Detections- Response variable
```{r}
...
## Light availability- Predictor variable 1
```{r}
...
## Territoriality- Preditor variable 2
```{r}
...
## Sociality- Predictor variable 3
```{r}
sociality <- sociality %>% dplyr::select (Species, Communal)
colnames (sociality) <- c("scientific_name", "sociality")

# Updating the scientific names of following species as per our dataset: see list in Chunk 5: 'Territoriality- Predictor variable 2'
...
## Peak frequency- Predictor variable 4
```{r}
...
vocal_act <- vocal_act %>% left_join(median_pf, by = c('eBird_codes', 'time_of_day')) %>%
  drop_na() 

# A total of 66 species were included and three species were excluded from final analysis.
```
 
## Predictor variable 5- Trophic niche
```{r}
vocal_act <- vocal_act %>% 
...
vocal_act <- vocal_act %>% mutate(trophic_niche = case_when (trophic_niche %in% 'Invertivore' ~ '1', trophic_niche %in% 'Frugivore' ~ '2', trophic_niche %in% 'Omnivore' ~'3', trophic_niche %in% 'Vertivore' ~ '4'))
```

## More data cleaning
```{r}
# converting the scientific names of vocal_act as per tip.labels of the tree
vocal_act <- vocal_act %>% mutate(across(scientific_name, str_replace, ' ', '_')) 
...
## PGLS analysis
```{r}
model <- gls(percent_detections ~ median_startTime + territory + sociality + median_peak_freq + trophic_niche, data = vocal_act, correlation = corPagel(1, pruned.tree, form = ~scientific_name))
