...
## Trophic niche: 1 = Invertivore, 2 = Frugivore, 3 = Omnivore
...
model1 <- vocal_act %>%
  filter(time_of_day == "dawn") %>%
  gls (percent_normalized_detections ~ scale (median_startTime), data = ., 
...
model2 <- vocal_act %>%
  filter(time_of_day == "dawn") %>% 
  gls(percent_normalized_detections ~ relevel(factor(territory), ref = 3), data = ., correlation = corPagel(1, pruned.tree, form = ~scientific_name))
...
model3 <- vocal_act %>%
  filter(time_of_day == "dawn") %>% 
  gls(percent_normalized_detections ~ factor(sociality), data = ., 
correlation = corPagel(1, pruned.tree, form = ~scientific_name))
...
model <- vocal_act %>%
  ...
  gls(percent_normalized_detections ~ scale(median_startTime) +
               ...
               relevel(factor(trophic_niche), ref = 2), data = ., 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))
