...
median_pf <- separate(freq_5, col = filename, into = c("site_id", "date", "time", "splits"), sep = "_") %>%
  ...
  summarise(median_peak_freq =  log10(median(peak_freq_in_Hz)))
...
vocal_act <- vocal_act %>% mutate(trophic_niche = case_when (trophic_niche %in% 'Invertivore' ~ '1', trophic_niche %in% 'Frugivore' ~ '2', trophic_niche %in% 'Omnivore' ~'3'))
...
model1 <- gls(percent_normalized_detections ~ scale (median_startTime), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))
summary (model1)

# percent_normalized_detections ~ territoriality only
model2 <- gls(percent_normalized_detections ~ relevel(factor(territory), ref = 3), data = vocal_act, correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model2)

# percent_normalized_detections ~ sociality only
model3 <- gls(percent_normalized_detections ~ factor(sociality), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model3)

# percent_normalized_detections ~ median_peak_freq only
model4 <- gls(percent_normalized_detections ~ scale (median_peak_freq), data = vocal_act, correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model4)

# percent_normalized_detections ~ trophic_niche only
model5 <- gls(percent_normalized_detections ~ relevel(factor(trophic_niche), ref = 1), data = vocal_act, correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model5)

# Multi-variate models with two predictor variables:
#percent_normalized_detections ~ median startTime + territoriality
model6 <- gls(percent_normalized_detections ~ scale (median_startTime) +
               relevel(factor(territory), ref = 3), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model6)

# percent_normalized_detections ~ median startTime + sociality
model7 <- gls(percent_normalized_detections ~ scale (median_startTime) +
               factor(sociality), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model7)

# percent_normalized_detections ~ median_startTime + median_peak_freq
model8 <- gls(percent_normalized_detections ~ scale (median_startTime) +
               scale (median_peak_freq), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model8)

# percent_normalized_detections ~ median startTime + trophic niche
model5 <- gls(percent_normalized_detections ~ scale (median_startTime) +
               relevel(factor(trophic_niche), ref = 1), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model5)

# percent_normalized_detections ~ territoriality + sociality
model9 <- gls(percent_normalized_detections ~ relevel(factor(territory), ref = 3) + 
               factor(sociality), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model9)

# percent_normalized_detections ~ territoriality + median_peak_freq
model10 <- gls(percent_normalized_detections ~ relevel(factor(territory), ref = 3) +                  scale (median_peak_freq), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model10)

# percent_normalized_detections ~ territoriality + trophic_niche
model11 <- gls(percent_normalized_detections ~ relevel(factor(territory), ref = 3) + 
               relevel(factor(trophic_niche), ref = 1), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model11)

# percent_normalized_detections ~ sociality + median_peak_freq
model12 <- gls(percent_normalized_detections ~ factor(sociality) +
               scale (median_peak_freq), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model12)

# percent_normalized_detections ~ sociality + trophic_niche
model13 <- gls(percent_normalized_detections ~ factor(sociality) +
               relevel(factor(trophic_niche), ref = 1), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))

summary(model13)

# percent_normalized_detections ~ median_peak_freq + trophic_niche
model14 <- gls(percent_normalized_detections ~ scale (median_peak_freq) + 
               relevel(factor(trophic_niche), ref = 1), data = vocal_act, 
             correlation = corPagel(1, pruned.tree, form = ~scientific_name))
...
model15 <- gls(percent_normalized_detections ~ scale (median_startTime) +
