...
# We will combine the foraging guild, territoriality and communal signalling figure into a side-by-side panel
# Assumption here that the foraging guild and territoriality figures are already loaded in the R environment (if not; please run the previous scripts for the same)

fig_trait <- wrap_plots(fig_between_foragingGuild_vocAct,
                      fig_terr_vocAct, fig_soc_vocAct,
  nrow = 2
) +
  plot_annotation(
    tag_levels = "a",
...
ggsave(fig_trait, filename = "figs/fig_percentDetections_vs_traits.png", width = 18, height = 16, device = png(), units = "in", dpi = 300)
dev.off()
```

![Significant differences in vocal activity were observed between (a) frugivores and invertivores and frugivores and omnivore species, and (b) between weakly territorial and non-territorial species.However, (c) no differences in vocal activity  between communal signallers and non-communal signallers were observed.](figs/fig_percentDetections_vs_traits.png)
