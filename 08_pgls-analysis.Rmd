...
pairs.panels(for_corr[,c(13,16:18)])
...
for(i in 1:long1){
model.i<-gls(percent_normalized_detections ~ scaled_median_startTime +
      territory +
      sociality +
      scaled_median_peak_freq +
      trophic_niche,
    data = data,,correlation=corPagel(lambda0[i], pruned.tree,form = ~scientific_name))
log.likes[i]<-logLik(model.i)[1] # y~x is your model, phy.tree your phylogeny
}
...
# R2_lik 
z.x <- gls(percent_normalized_detections ~ 1, data = data, correlation = corBrownian(1, pruned.tree, form = ~scientific_name))

R2_lik(model6)
R2_lik(model6, z.x)

terrEff <- emmeans(model6, ~"territory", data = data)
pairs(terrEff)
eff_size(terrEff, sigma = sigma(model6), edf = 39)
plot(terrEff, comparisons = TRUE)
pwpp(terrEff)

nicheEff <- emmeans(model6, ~"trophic_niche", data = data)
pairs(nicheEff)
eff_size(nicheEff, sigma = sigma(model6), edf = 39)
